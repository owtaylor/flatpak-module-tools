...
class FlatpakSourceInfo(object):
    ...
    def app_modules(self):
        ...
        def is_runtime(m):
            ...
            return runtime_module_name in dependencies[0].props.requires

        return [m for m in self.modules.values() if is_runtime(m)]


class FlatpakBuilder(object):
    def __init__(self, source, workdir, root='var/tmp/flatpak-build'):
        self.source = source
    ...
    def get_includepkgs(self):
        ...
        if not source.runtime:
            ...
            available_packages = sorted(runtime_module.mmd.props.profiles['runtime'].props.rpms.get())
